...
var bright_ = preload("res://brightness.gdshader")
...
func create_effect_layer(effect_type: String):
	var layer = CanvasLayer.new()
	var colRect = ColorRect.new()
	colRect.material = ShaderMaterial.new()
	if effect_type == "Saturation":
		print("set Saturation shader")
		colRect.material.shader = sat
		colRect.material.set_shader_parameter("saturation",-10.0)
	if effect_type == "Contrast":
		print("set Contrast shader")
		colRect.material.shader = contr
		colRect.material.set_shader_parameter("contrast",1.0)
	if effect_type == "Brightness":
		print("set Brightness shader")
		colRect.material.shader = bright_
		colRect.material.set_shader_parameter("brightness",1.0)
	if effect_type == "Sharpness":
		print("set Sharpness shader")
		colRect.material.shader = sharp
		colRect.material.set_shader_parameter("sharpness",1.0)
	layer.add_child(colRect)
	layer.add_to_group(str(get_child_count()))
...
func apply_mask_effects():
	...
	for effect_index in len(mask_effect_order):
		var effect: Effect = mask_effect_order[effect_index]

		var current = viewport_rects[current_viewport_rect]


		await RenderingServer.frame_post_draw
		current.set_texture(test_texture)
		#current_viewport = 1 - current_viewport
		current_viewport_rect = 1 - current_viewport_rect
		current = viewport_rects[current_viewport_rect]
		current.set_texture(viewports[1-current_viewport].get_texture())
		current.material.shader = effect.shader#bright_
		current.material.set_shader_parameter("text", MaskDrawingMaster.get_texture_by_name(effect.mask_name))
		for shader_val in effect.values:
			print("shader_valBBBBB")
			current.material.set_shader_parameter(shader_val, effect.values[shader_val])
		...
		#await RenderingServer.frame_post_draw
		#current_viewport = 1 - current_viewport
	texture = viewports[current_viewport].get_texture()
	#emit_signal("finished_apply_effect")
...
#		return
#	var s = Time.get_ticks_msec()
#	texture = %lastDrawnTextureVP.get_texture()
#	is_drawing = true
#	ttt.texture =  current_texture # apply current teture first
#
#	vp.set_update_mode(SubViewport.UPDATE_ONCE)
#	for val in effect_order[0].values:
#		ttt.material.set_shader_parameter(val, effect_order[0].values[val])
#	ttt.material.shader = effect_order[0].shader
#	await RenderingServer.frame_post_draw
#
#	ttt2.texture =  vp.get_texture() # apply current teture first
#
#	vp2.set_update_mode(SubViewport.UPDATE_ONCE)	
#	for val in effect_order[1].values:
#		ttt2.material.set_shader_parameter(val, effect_order[1].values[val])
#	ttt2.material.shader = effect_order[1].shader
#	current_viewport = 0
#	current_viewport_rect = 1
#
#	await RenderingServer.frame_post_draw
#
#
#
#	vp.set_update_mode(SubViewport.UPDATE_ONCE)
#	ttt.set_texture(vp2.get_texture())
#	for val in effect_order[2].values:
#		ttt.material.set_shader_parameter(val, effect_order[2].values[val])
#	ttt.material.shader = effect_order[2].shader
#	await RenderingServer.frame_post_draw
#
#	vp2.set_update_mode(SubViewport.UPDATE_ONCE)
#	ttt2.set_texture(vp.get_texture())
#	for val in effect_order[3].values:
#		ttt2.material.set_shader_parameter(val, effect_order[3].values[val])
#	ttt2.material.shader = effect_order[3].shader
#	await RenderingServer.frame_post_draw
#
#	%lastDrawnTextureVP.set_update_mode(SubViewport.UPDATE_ONCE)
#	texture = vp2.get_texture()
#	%lastDrawnTexture.texture = texture
#
#	is_drawing = false
#
#	texture = %lastDrawnTextureVP.get_texture()
